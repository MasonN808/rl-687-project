...
def QL(env, alpha, epsilon = 0.02, delta = 1e-3):
    Q = np.ones((5, 5, 4))*5  
    Q[4,4] = 0
    Q[2,2] = 0
    Q[3,2] = 0
...
if __name__ == "__main__":
    ...
    plt.title("QL: Actions vs. Episodes")
    plt.savefig("QL:AE")
    plt.close()
    ...
    plt.ylabel("MSE")
    plt.title("QL: Episodes vs. MSE")
    plt.savefig("QL:ME")
    plt.close()
    drawPolicy(np.argmax(Q, axis=2),"QL","QL Policy", {'G':[(4,4)],'E':[(2,2),(3,2)]})
